{{-- file: resources/views/vpc_sessions/index.blade.php --}}
@extends('layouts.app')
    ...
    <header class="bg-white border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <a href="{{ route('vpc_sessions.create') }}"
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if($sessions->isEmpty())
            <div class="text-center py-12">
                ...
                <a href="{{ route('vpc_sessions.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                    Создать сессию
                </a>
            </div>
        @else
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($sessions as $session)
                            <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location='{{ route('vpc_sessions.show', $session) }}'">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    #{{ $session->id }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $session->name ?? 'Без названия' }}</div>
                                    @if($session->agentTask)
                                        <div class="text-sm text-gray-500">Задача: {{ $session->agentTask->title }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                    ...
                                    @endphp
                                    <span class="px-2 py-1 text-xs font-medium rounded-full {{ $statusColors[$session->status] ?? 'bg-gray-100 text-gray-800' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('vpc_sessions.show', $session) }}" class="text-blue-600 hover:text-blue-900">
                                        Открыть
                                    </a>
                                </td>
                            </tr>
