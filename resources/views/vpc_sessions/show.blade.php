{{-- file: resources/views/vpc_sessions/show.blade.php --}}
@extends('layouts.app')
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center space-x-4">
                    <a href="{{ route('vpc_sessions.index') }}" class="text-gray-500 hover:text-gray-700">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                        </svg>
                    </a>
                    <div>
                        <h1 class="text-xl font-semibold text-gray-900">{{ $session->name ?? 'VPC-сессия #' . $session->id }}</h1>
                        @if($session->agentTask)
                            <p class="text-sm text-gray-500">Задача: {{ $session->agentTask->title }}</p>
                        @endif
                    </div>
                </div>
                <div class="flex items-center space-x-3">
                    ...
                    @endphp
                    <span class="px-3 py-1 text-sm font-medium rounded-full {{ $statusColors[$session->status] ?? 'bg-gray-100 text-gray-800' }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <div class="p-4 border-b border-gray-200 flex justify-between items-center">
                        ...
                        @endphp
                        <span class="px-2 py-1 text-xs font-medium rounded-full {{ $modeColors[$session->control_mode] ?? 'bg-gray-100 text-gray-800' }}">
                            {{ $modeNames[$session->control_mode] ?? $session->control_mode }}
                        </span>
                    </div>
                    ...
                    <div class="aspect-video bg-gray-900 flex items-center justify-center">
                        @if($session->status === 'running')
                            <div class="text-center">
                                ...
                                <p class="text-gray-500 text-sm">Endpoint: {{ $session->endpoint ?? 'Не задан' }}</p>
                            </div>
                        @else
                            <div class="text-center">
                                <svg class="w-16 h-16 text-gray-600 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Control Panel -->
                    @if($session->control_mode === 'USER_CONTROL' && $session->status === 'running')
                    ...
                    @else
                        <div class="space-y-2 max-h-64 overflow-y-auto">
                            @foreach($actions as $action)
                                <div class="flex items-start text-sm border-l-4 pl-3 py-1 {{ $action->source === 'agent' ? 'border-purple-500 bg-purple-50' : 'border-blue-500 bg-blue-50' }} rounded-r">
                                    <span class="text-xs text-gray-400 w-20 flex-shrink-0">
                                        {{ $action->created_at->format('H:i:s') }}
                                    </span>
                                    <span class="font-medium {{ $action->source === 'agent' ? 'text-purple-700' : 'text-blue-700' }} w-16 flex-shrink-0">
                                        {{ $action->source === 'agent' ? 'Агент' : 'User' }}
                                    </span>
                                    <span class="text-gray-700">
                                        {{ $action->action_type }}
                                        @if($action->payload)
                                            <span class="text-gray-500">{{ json_encode($action->payload) }}</span>
                                        @endif
                                    </span>
                                </div>
                            @endforeach
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <dl class="space-y-3 text-sm">
                        <div class="flex justify-between">
                            ...
                            <dd class="text-gray-900">{{ $session->id }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">Статус</dt>
                            <dd class="text-gray-900">{{ $statusNames[$session->status] ?? $session->status }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">Режим</dt>
                            <dd class="text-gray-900">{{ $modeNames[$session->control_mode] ?? $session->control_mode }}</dd>
                        </div>
                        <div class="flex justify-between">
                        ...
                        @if($session->started_at)
                            <div class="flex justify-between">
                                ...
                                <dd class="text-gray-900">{{ $session->started_at->format('d.m.Y H:i') }}</dd>
                            </div>
                        @endif
                        @if($session->last_activity_at)
                            <div class="flex justify-between">
                                <dt class="text-gray-500">Активность</dt>
                                <dd class="text-gray-900">{{ $session->last_activity_at->diffForHumans() }}</dd>
                            </div>
                        @endif
                    </dl>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-2">
                        <form action="{{ route('vpc_sessions.control_mode', $session) }}" method="POST">
                            @csrf
                            <input type="hidden" name="mode" value="AGENT_CONTROL">
                            <button type="submit"
                                    class="w-full px-4 py-2 text-left rounded-lg border {{ $session->control_mode === 'AGENT_CONTROL' ? 'bg-purple-50 border-purple-300' : 'border-gray-300 hover:bg-gray-50' }}">
                        ...
                        <form action="{{ route('vpc_sessions.control_mode', $session) }}" method="POST">
                            ...
                            <button type="submit"
                                    class="w-full px-4 py-2 text-left rounded-lg border {{ $session->control_mode === 'USER_CONTROL' ? 'bg-blue-50 border-blue-300' : 'border-gray-300 hover:bg-gray-50' }}">
                                <div class="flex items-center">
                        ...
                        <form action="{{ route('vpc_sessions.control_mode', $session) }}" method="POST">
                            ...
                            <button type="submit"
                                    class="w-full px-4 py-2 text-left rounded-lg border {{ $session->control_mode === 'PAUSED' ? 'bg-gray-100 border-gray-400' : 'border-gray-300 hover:bg-gray-50' }}">
                                <div class="flex items-center">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-2">
                        ...
                        @if($session->status === 'running')
                            <form action="{{ route('vpc_sessions.stop', $session) }}" method="POST">
                                @csrf
                        ...
                        <form action="{{ route('vpc_sessions.destroy', $session) }}" method="POST"
