...
<head>
    ...
    <title>Накладная - Заказ #{{ $order->order_id }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .document-title { font-size: 18px; font-weight: bold; text-transform: uppercase; margin-bottom: 5px; }
        .document-number { font-size: 14px; margin-bottom: 15px; }
        .info-section { display: flex; justify-content: space-between; margin-bottom: 20px; padding: 10px; background: #f9f9f9; border: 1px solid #ddd; }
        .info-block { width: 48%; }
        .info-row { display: flex; margin-bottom: 5px; }
        .info-label { width: 110px; font-weight: bold; color: #666; }
        .info-value { flex: 1; }
        .items-table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
        .items-table th { background: #e0e0e0; border: 1px solid #333; padding: 8px; text-align: center; font-size: 11px; font-weight: bold; }
        .items-table td { border: 1px solid #333; padding: 6px 8px; font-size: 11px; }
        .items-table .num { text-align: center; width: 30px; }
        .items-table .sku { text-align: center; width: 100px; }
        ...
        .items-table .sum { text-align: right; width: 90px; }
        .totals { display: flex; justify-content: flex-end; margin-bottom: 30px; }
        .totals-box { border: 2px solid #333; padding: 15px; width: 250px; }
        .totals-row { display: flex; justify-content: space-between; margin-bottom: 5px; }
        .totals-row.total { font-size: 14px; font-weight: bold; border-top: 1px solid #333; padding-top: 10px; margin-top: 10px; }
        .summary { margin-bottom: 30px; padding: 10px; border: 1px solid #ddd; background: #fafafa; }
        .signatures { display: flex; justify-content: space-between; margin-top: 40px; }
        .signature-block { width: 30%; text-align: center; }
        .signature-title { font-weight: bold; margin-bottom: 5px; font-size: 11px; }
        .signature-line { border-bottom: 1px solid #333; margin: 25px 0 5px 0; }
        .signature-hint { font-size: 9px; color: #666; }
...
<body>
    <div class="header">
        ...
        <div class="document-number">№ YM-{{ $order->order_id }} от {{ $order->created_at_ym?->format('d.m.Y') ?? $order->created_at->format('d.m.Y') }}</div>
    </div>
    ...
    <div class="info-section">
        ...
        <div class="info-block">
            <div class="info-row">
                ...
                <span class="info-value">{{ $order->customer_name ?? 'Покупатель YM' }}</span>
            </div>
            ...
            <div class="info-row">
                ...
                <span class="info-value">{{ $order->delivery_type }}</span>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="info-section" style="background: #fff;">
        <div class="info-block">
            <div class="info-row">
                ...
                <span class="info-value">Yandex Market</span>
            </div>
            <div class="info-row">
    ...
    <table class="items-table">
        ...
        <tbody>
            @foreach($items as $item)
                ...
                <tr>
                    <td class="num">{{ $num++ }}</td>
                    <td class="sku">{{ $item['offerId'] ?? $item['shopSku'] ?? '—' }}</td>
        ...
        <tfoot>
            <tr>
                ...
                <td class="qty" style="font-weight: bold;">{{ $totalQty }}</td>
                <td></td>
                <td></td>
                <td class="sum" style="font-weight: bold;">{{ number_format((float)$order->total_price, 2, '.', ' ') }}</td>
            </tr>
        </tfoot>
    </table>

    <div class="summary">
        <strong>Всего отпущено {{ $totalQty }} наименований на сумму {{ number_format((float)$order->total_price, 2, '.', ' ') }} {{ $order->currency ?? 'RUR' }}</strong>
    </div>
