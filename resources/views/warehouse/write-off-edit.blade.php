...
@section('content')
{{-- BROWSER MODE --}}
<div class="browser-only flex h-screen bg-gradient-to-br from-slate-50 to-red-50" x-data="writeOffEditPage({{ $documentId }})"
     :class="{
    ...
    <div class="flex-1 flex flex-col overflow-hidden"
        ...
        <header class="bg-white/80 backdrop-blur-sm border-b border-gray-200/50 px-6 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold bg-gradient-to-r from-red-600 to-orange-600 bg-clip-text text-transparent">Редактирование списания</h1>
                    <p class="text-sm text-gray-500" x-text="'Документ ' + (doc?.doc_no || '#{{ $documentId }}')"></p>
                </div>
                <div class="flex items-center space-x-3">
                    <button class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl transition-colors" @click="back()">Отмена</button>
        ...
        <main x-show="!loadingDoc && doc && doc.status === 'DRAFT'" class="flex-1 overflow-y-auto px-6 py-6 space-y-6">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <div class="flex items-start justify-between mb-6">
                    <div>
                        <div class="text-sm text-gray-500 mb-1">Списание №</div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                        ...
                        <select class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-red-500 focus:border-red-500" x-model="form.warehouse_id">
                    ...
                    <div>
                        ...
                        <select class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-red-500 focus:border-red-500" x-model="form.reason" @change="onReasonChange()">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        Комментарий <span x-show="requiresComment" class="text-red-500">*</span>
                    </label>
                    <input type="text" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-red-500 focus:border-red-500" x-model="form.comment" :placeholder="requiresComment ? 'Обязательно' : 'Необязательно'">
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100">
                <div class="px-6 py-4 border-b flex items-center justify-between bg-gray-50">
                    <div>
                        <h2 class="text-lg font-semibold text-gray-900">Позиции для списания</h2>
                        <p class="text-sm text-gray-500" x-text="form.lines.length + ' позиций'"></p>
                    </div>
                    <div class="flex items-center space-x-2">
                        <button class="px-4 py-2 bg-red-100 hover:bg-red-200 text-red-700 rounded-xl transition-colors text-sm font-medium" @click="addLine()">+ Добавить</button>
                        <button class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl transition-colors text-sm" @click="clearLines()">Очистить</button>
                    </div>
                </div>
                    ...
                    <template x-for="(line, idx) in form.lines" :key="idx">
                        <div class="grid grid-cols-1 md:grid-cols-6 gap-4 px-6 py-4 hover:bg-gray-50 transition-colors">
                            <div class="relative md:col-span-2">
                                ...
                                <input type="text" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                       :value="line.search || ''" @input="line.search = $event.target.value; searchSku(idx)" placeholder="Поиск по SKU, штрихкоду, названию">
                                <div class="absolute z-[100] mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-64 overflow-y-auto"
                                    ...
                                    <template x-for="item in line.suggestions" :key="item.sku_id">
                                        <div class="flex items-center gap-3 px-3 py-2 hover:bg-red-50 cursor-pointer border-b border-gray-100 last:border-0" @click="selectSku(idx, item)">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <div class="text-xs text-gray-500" x-text="'SKU: ' + item.sku_code"></div>
                                            </div>
                                            <div class="text-sm font-semibold" :class="item.available > 0 ? 'text-green-600' : 'text-gray-400'" x-text="item.available || 0"></div>
                                        </div>
                                    </template>
                                </div>
                                <div class="absolute z-[100] mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg px-4 py-3" x-show="line.noResults" x-cloak>
                                    <div class="text-sm text-gray-500 text-center">Товары не найдены</div>
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Текущий остаток</label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Кол-во списания *</label>
                            ...
                            <div>
                                ...
                                <input type="text" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-red-500 focus:border-red-500" x-model="line.note" placeholder="Необязательно">
                            </div>
                            <div class="flex items-end">
                                <button class="px-4 py-2.5 bg-red-100 hover:bg-red-200 text-red-600 rounded-xl transition-colors text-sm" @click="removeLine(idx)" :disabled="form.lines.length===1">Удалить</button>
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="text-center p-4 bg-red-50 rounded-xl">
                        ...
                        <div class="text-sm text-gray-500">Кол-во</div>
                    </div>
    ...
    function writeOffEditPage(docId) {
        return {
            ...
            async loadDocument() {
                ...
                try {
                    const resp = await fetch(`/api/marketplace/inventory/documents/${docId}`, { headers: this.getAuthHeaders() });
                    ...
                    this.form.reason = this.doc.reason || '';
            ...
            back() { window.location.href = `/warehouse/documents/${docId}`; },

            itemsCount() { return this.form.lines.filter(l => l.sku_id).length; },
            totalQty() { return this.form.lines.reduce((acc, l) => acc + (Number(l.qty) || 0), 0); },
            totalCost() {
            ...
            async save(postNow = false) {
                this.error = '';
                if (!this.form.reason) { this.error = 'Выберите причину списания'; return; }
                if (this.requiresComment && !this.form.comment.trim()) { this.error = 'Комментарий обязателен для этой причины'; return; }

                const validLines = this.form.lines.filter(l => l.sku_id && l.qty > 0);
                if (validLines.length === 0) { this.error = 'Добавьте хотя бы одну позицию'; return; }

                const authStore = this.$store.auth;
                if (!authStore?.currentCompany) { this.error = 'Нет активной компании'; return; }
                ...
                try {
                    ...
                    if (!updateResp.ok || updateJson.errors) throw new Error(updateJson.errors?.[0]?.message || 'Ошибка обновления');
                    ...
                    // 3. Провести
                    if (postNow) {
                        const postResp = await fetch(`/api/marketplace/inventory/documents/${docId}/post`, { method: 'POST', headers });
                        const postJson = await postResp.json();
                        if (!postResp.ok || postJson.errors) throw new Error(postJson.errors?.[0]?.message || 'Ошибка проведения');
                    }

                    this.showToast(postNow ? 'Списание проведено' : 'Списание сохранено', 'success');
                    ...
                    if (resp.ok) {
                        ...
                        line.suggestions = (json.data?.items || [])
                            ...
                            .map(item => ({
                                ...
                                available: item.available, unit_cost: item.unit_cost || 0
                            }));
...
{{-- PWA MODE --}}
<div class="pwa-only min-h-screen" x-data="writeOffEditPage({{ $documentId }})" style="background: #f2f2f7;">
    <x-pwa-header title="Ред. списания" :backUrl="'/warehouse/documents/' . $documentId">
        <button @click="save(false)" :disabled="saving" class="native-header-btn text-red-600">
            <span x-show="!saving">Сохранить</span>
            <span x-show="saving">...</span>
        </button>
    </x-pwa-header>
    ...
    <main class="native-scroll" style="padding-top: calc(44px + env(safe-area-inset-top, 0px)); padding-bottom: calc(70px + env(safe-area-inset-bottom, 0px)); padding-left: calc(12px + env(safe-area-inset-left, 0px)); padding-right: calc(12px + env(safe-area-inset-right, 0px)); min-height: 100vh;">
        ...
        <div x-show="loadingDoc" class="px-4 py-8 text-center">
            <svg class="animate-spin w-6 h-6 text-red-600 mx-auto" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
        ...
        <div x-show="!loadingDoc && doc && doc.status === 'DRAFT'" class="px-4 py-4 space-y-4">
            <div class="native-card space-y-3">
                ...
                <div>
                    <label class="native-caption">Комментарий <span x-show="requiresComment" class="text-red-500">*</span></label>
                    <input type="text" class="native-input mt-1" x-model="form.comment" :placeholder="requiresComment ? 'Обязательно' : 'Необязательно'">
                </div>
            </div>

            <div class="native-card">
                <div class="flex items-center justify-between mb-3">
                    <p class="native-body font-semibold">Позиции</p>
                    <button class="text-red-600 font-medium text-sm" @click="addLine()">+ Добавить</button>
                </div>
                <div class="space-y-3">
                    <template x-for="(line, idx) in form.lines" :key="idx">
                        <div class="p-3 bg-gray-50 rounded-xl space-y-2">
                            <div class="relative">
                                <input type="text" class="native-input" :value="line.search || ''" @input="line.search = $event.target.value; searchSku(idx)" placeholder="SKU / штрихкод / название">
                                <div class="absolute z-[100] mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg max-h-64 overflow-y-auto"
                                     x-show="line.suggestions && line.suggestions.length > 0" x-cloak>
                                    <template x-for="item in line.suggestions" :key="item.sku_id">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <div>
                                    <label class="native-caption">Остаток</label>
