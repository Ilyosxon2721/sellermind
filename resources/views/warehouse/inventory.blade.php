...
@section('content')
{{-- BROWSER MODE --}}
<div class="browser-only flex h-screen bg-gradient-to-br from-slate-50 to-purple-50" x-data="inventoryPage()"
     :class="{
    ...
    <div class="flex-1 flex flex-col overflow-hidden"
        ...
        <header class="bg-white/80 backdrop-blur-sm border-b border-gray-200/50 px-6 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold bg-gradient-to-r from-purple-600 to-violet-600 bg-clip-text text-transparent">Инвентаризация</h1>
                    <p class="text-sm text-gray-500">Проверка фактических остатков на складе</p>
                </div>
                <div class="flex items-center space-x-3">
                    ...
                    <a href="/warehouse/inventory/create" class="px-4 py-2 bg-gradient-to-r from-purple-500 to-violet-500 hover:from-purple-600 hover:to-violet-600 text-white rounded-xl transition-all shadow-lg shadow-purple-500/25 flex items-center space-x-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                        <span>Создать инвентаризацию</span>
                    </a>
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 overflow-y-auto px-6 py-6 space-y-6">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-purple-500 focus:border-purple-500" x-model="filters.from">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Дата по</label>
                        <input type="date" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-purple-500 focus:border-purple-500" x-model="filters.to">
                    </div>
                    <div class="flex items-end">
                        <button class="w-full px-4 py-2.5 bg-purple-600 hover:bg-purple-700 text-white rounded-xl transition-colors font-medium" @click="load()">
                            Применить
                        </button>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 flex items-center space-x-4">
                    ...
                    <div>
                        <div class="text-2xl font-bold text-gray-900" x-text="totalLinesChecked()">0</div>
                        <div class="text-sm text-gray-500">Позиций проверено</div>
                    </div>
                </div>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Позиций</th>
                        ...
                        <template x-if="!loading && items.length === 0">
                            <tr><td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <div class="text-gray-500 mb-2">Документы инвентаризации не найдены</div>
                                <a href="/warehouse/inventory/create" class="text-purple-600 hover:text-purple-700 font-medium">Создать первую инвентаризацию &rarr;</a>
                            </td></tr>
                        </template>
                        <template x-for="doc in items" :key="doc.id">
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4">
                                    <a :href="`/warehouse/documents/${doc.id}`" class="font-semibold text-purple-600 hover:text-purple-700 hover:underline" x-text="doc.doc_no"></a>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <span class="px-3 py-1 rounded-full text-xs font-medium"
                                          :class="doc.status === 'POSTED' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'"
                                          x-text="doc.status === 'POSTED' ? 'Проведён' : 'Черновик'"></span>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-700 font-medium" x-text="(doc.lines_count || 0) + ' поз.'"></td>
                                <td class="px-6 py-4 text-right space-x-2">
                                    ...
                                    <button class="inline-flex items-center px-3 py-1.5 bg-purple-100 hover:bg-purple-200 text-purple-700 rounded-lg text-sm transition-colors"
    ...
    function inventoryPage() {
        return {
            ...
            getAuthHeaders() {
                ...
                // Fallback: check _x_auth_token (Alpine persist format)
            ...
            totalLinesChecked() {
                return this.items.reduce((sum, d) => sum + (d.lines_count || 0), 0);
            },
            ...
            async postDoc(id) {
                try {
                    ...
                    this.showToast('Инвентаризация проведена', 'success');
                    this.load();
                } catch(e) {
...
<div class="pwa-only min-h-screen" x-data="inventoryPage()" style="background: #f2f2f7;">
    <x-pwa-header title="Инвентаризация" :backUrl="'/warehouse'">
        ...
        <a href="/warehouse/inventory/create" class="native-header-btn text-purple-600" onclick="if(window.haptic) window.haptic.light()">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <main class="native-scroll" style="padding-top: calc(44px + env(safe-area-inset-top, 0px)); padding-bottom: calc(70px + env(safe-area-inset-bottom, 0px)); padding-left: calc(12px + env(safe-area-inset-left, 0px)); padding-right: calc(12px + env(safe-area-inset-right, 0px)); min-height: 100vh;" x-pull-to-refresh="load">
        ...
        <div class="px-4 py-4">
            <div class="native-card space-y-3">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label class="native-caption">Дата с</label>
                        <input type="date" class="native-input mt-1" x-model="filters.from">
                    </div>
                    <div>
                        <label class="native-caption">Дата по</label>
                        <input type="date" class="native-input mt-1" x-model="filters.to">
                    </div>
                </div>
                <button class="native-btn w-full bg-purple-600" @click="load()">Применить</button>
            </div>
        </div>
        ...
        <div class="px-4 grid grid-cols-2 gap-2 mb-4">
            ...
            <div class="native-card text-center py-3">
                <p class="text-2xl font-bold text-blue-600" x-text="items.filter(d => d.status === 'POSTED').length">0</p>
            ...
            <div class="native-card text-center py-3">
                <p class="text-2xl font-bold text-purple-600" x-text="totalLinesChecked()">0</p>
                <p class="native-caption">Позиций проверено</p>
            </div>
        </div>
        ...
        <div x-show="!loading && items.length === 0" class="px-4">
            <div class="native-card text-center py-12">
                ...
                <a href="/warehouse/inventory/create" class="text-purple-600 font-medium">Создать первую инвентаризацию &rarr;</a>
            </div>
        </div>
            ...
            <template x-for="doc in items" :key="doc.id">
                <a :href="`/warehouse/documents/${doc.id}`" class="native-card block">
                    <div class="flex items-start justify-between mb-2">
                        <p class="native-body font-semibold text-purple-600" x-text="doc.doc_no"></p>
                        <span class="text-xs px-2 py-0.5 rounded-full" :class="doc.status === 'POSTED' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'" x-text="doc.status === 'POSTED' ? 'Проведён' : 'Черновик'"></span>
                    </div>
                    <div class="flex items-center justify-between">
                        <p class="native-caption" x-text="doc.warehouse?.name || '—'"></p>
                        <p class="native-caption font-medium text-purple-600" x-text="(doc.lines_count || 0) + ' поз.'"></p>
                    </div>
