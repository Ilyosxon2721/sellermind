...
@section('content')
{{-- BROWSER MODE --}}
<div class="browser-only flex h-screen bg-gradient-to-br from-slate-50 to-purple-50" x-data="inventoryEditPage({{ $documentId }})"
     :class="{
    ...
    <div class="flex-1 flex flex-col overflow-hidden"
        ...
        <header class="bg-white/80 backdrop-blur-sm border-b border-gray-200/50 px-6 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold bg-gradient-to-r from-purple-600 to-violet-600 bg-clip-text text-transparent">Редактирование инвентаризации</h1>
                    <p class="text-sm text-gray-500" x-text="'Документ ' + (doc?.doc_no || '#{{ $documentId }}')"></p>
                </div>
                <div class="flex items-center space-x-3">
                    <button class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl transition-colors" @click="back()">Назад</button>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                                x-model="form.warehouse_id" @change="loadBalances()">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-[3fr_1fr_1fr_1fr_1fr] gap-4 px-6 py-4 hover:bg-gray-50/50 transition-colors"
                                ...
                                <div class="flex items-center justify-center">
                                    <template x-if="line.counted_qty !== null && line.counted_qty !== ''">
                                        <span class="text-sm font-bold"
                                              :class="{ 'text-green-600': line.diff > 0, 'text-red-600': line.diff < 0, 'text-gray-400': line.diff === 0 }"
                                              x-text="line.diff > 0 ? '+' + line.diff : (line.diff === 0 ? '0' : line.diff)"></span>
                                    </template>
                                    <template x-if="line.counted_qty === null || line.counted_qty === ''">
                                ...
                                <div class="flex items-center justify-end">
                                    <template x-if="line.counted_qty !== null && line.counted_qty !== '' && line.diff !== 0">
                                        <span class="text-sm font-medium" :class="line.diff < 0 ? 'text-red-600' : 'text-green-600'"
                                              x-text="(Math.abs(line.diff) * line.unit_cost).toLocaleString('ru-RU', {minimumFractionDigits: 2, maximumFractionDigits: 2})"></span>
                                    </template>
                                    <template x-if="line.counted_qty === null || line.counted_qty === '' || line.diff === 0">
                                        <span class="text-sm text-gray-300">&mdash;</span>
                                    </template>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="text-center p-4 bg-violet-50 rounded-xl">
                        <div class="text-2xl font-bold"
                             :class="{ 'text-green-600': totalDifference() > 0, 'text-red-600': totalDifference() < 0, 'text-gray-400': totalDifference() === 0 }"
...
<script nonce="{{ $cspNonce ?? '' }}">
    function inventoryEditPage(docId) {
        return {
            ...
            savedLines: [], // Строки из документа для восстановления counted_qty
            ...
            async loadDocument() {
                ...
                try {
                    ...
                    // Сохраняем строки документа для использования при загрузке балансов
                    this.savedLines = lines.map(l => ({
                        sku_id: l.sku_id,
                        counted_qty: parseInt(l.qty),
                    }));
            ...
            async loadBalances() {
                if (!this.form.warehouse_id) { this.form.lines = []; return; }

                this.loadingBalances = true;
                this.error = '';

                try {
                    const params = new URLSearchParams({ warehouse_id: this.form.warehouse_id, per_page: 500 });
                    const resp = await fetch(`/api/marketplace/stock/balance?${params.toString()}`, { headers: this.getAuthHeaders() });
                    const json = await resp.json();
                    if (!resp.ok || json.errors) throw new Error(json.errors?.[0]?.message || 'Ошибка загрузки остатков');

                    const items = json.data?.items || [];
                    this.form.lines = items.map(item => {
                        // Восстанавливаем counted_qty из сохранённых строк документа
                        const saved = this.savedLines.find(s => s.sku_id === item.sku_id);
                        const counted = saved ? saved.counted_qty : null;
                        const expected = item.on_hand || 0;
                        return {
                            sku_id: item.sku_id,
                            ...
                            counted_qty: counted,
                            diff: counted !== null ? counted - expected : 0,
            ...
            countedCount() { return this.form.lines.filter(l => l.counted_qty !== null && l.counted_qty !== '').length; },
            matchCount() { return this.form.lines.filter(l => l.counted_qty !== null && l.counted_qty !== '' && l.diff === 0).length; },
            discrepancyCount() { return this.form.lines.filter(l => l.counted_qty !== null && l.counted_qty !== '' && l.diff !== 0).length; },
            totalDifference() {
                return this.form.lines.filter(l => l.counted_qty !== null && l.counted_qty !== '').reduce((sum, l) => sum + l.diff, 0);
            },
            ...
            async save(postNow = false) {
                this.error = '';
                if (!this.form.warehouse_id) { this.error = 'Выберите склад'; return; }

                const validLines = this.form.lines.filter(l => l.counted_qty !== null && l.counted_qty !== '');
                if (validLines.length === 0) { this.error = 'Введите фактическое количество хотя бы для одной позиции'; return; }
...
{{-- PWA MODE --}}
<div class="pwa-only min-h-screen" x-data="inventoryEditPage({{ $documentId }})" style="background: #f2f2f7;">
    <x-pwa-header title="Ред. инвентаризации" :backUrl="'/warehouse/documents/' . $documentId">
        <button @click="save(true)" :disabled="saving" class="native-header-btn text-purple-600">
            <span x-show="!saving">Провести</span>
    ...
    <main class="native-scroll" style="padding-top: calc(44px + env(safe-area-inset-top, 0px)); padding-bottom: calc(70px + env(safe-area-inset-bottom, 0px)); padding-left: calc(12px + env(safe-area-inset-left, 0px)); padding-right: calc(12px + env(safe-area-inset-right, 0px)); min-height: 100vh;">
        ...
        <div x-show="loadingDoc" class="px-4 py-8 text-center">
            <svg class="animate-spin w-6 h-6 text-purple-600 mx-auto" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
        ...
        <div x-show="!loadingDoc && doc && doc.status === 'DRAFT'" class="px-4 py-4 space-y-4">
            ...
            <div x-show="form.lines.length > 0" class="native-card">
                <div class="relative">
                    ...
                    <input type="text" class="native-input pl-9" x-model="searchQuery" placeholder="Поиск по SKU, названию...">
                </div>
            </div>
                    ...
                    <div class="native-card"
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            ...
                            <div>
                                ...
                                <input type="number" step="1" min="0" class="native-input mt-1 text-center font-medium"
                                       :value="line.counted_qty" @input="updateCountedQty(line, $event.target.value)" placeholder="—">
                            </div>
                            <div>
