            ...
            @foreach($store->visibleCategories as $cat)
                <a href="/store/{{ $slug }}/catalog?category={{ $cat->id }}"
                   class="flex-shrink-0 flex flex-col items-center gap-2 group">
                    <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-purple-50 to-indigo-50 flex items-center justify-center group-hover:shadow-md transition-shadow">
                        @if($cat->custom_image)
