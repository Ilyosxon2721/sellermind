...
<div class="group bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg hover:border-gray-200 transition-all duration-200 flex flex-col">
    ...
    <a href="/store/{{ $slug }}/product/{{ $sp->id }}" class="relative block aspect-square overflow-hidden bg-gray-50">
        @if($image)
            <img src="{{ $image }}" alt="{{ $name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300 {{ !$inStock ? 'opacity-50' : '' }}" loading="lazy">
        @else
    ...
    <div class="flex flex-col flex-1 p-3">
        ...
        @if($inStock)
            <button
                ...
                class="mt-2 w-full py-2 rounded-lg text-sm font-semibold text-white transition-all duration-200 hover:opacity-90 active:scale-95"
