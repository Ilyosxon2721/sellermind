...
<body>
    <div class="header">
        ...
        <div class="document-number">№ {{ $order['order_number'] }} от {{ $order['date']->format('d.m.Y') }}</div>
        <div class="marketplace-info">
            <span class="marketplace-badge marketplace-{{ $order['marketplace'] }}">
                {{ $order['marketplace_label'] }}
            </span>
            <span class="marketplace-account">Аккаунт: {{ $order['account_name'] }}</span>
        </div>
    </div>
    ...
    <div class="info-section">
        ...
        <div class="info-block">
            <div class="info-row">
                ...
                <span class="info-value">{{ $order['customer_name'] ?? 'Розничный покупатель' }}</span>
            </div>
    ...
    <div class="info-section" style="background: #fff;">
        <div class="info-block">
            <div class="info-row">
                ...
                <span class="info-value">{{ $order['marketplace_label'] }}</span>
            </div>
        </div>
        <div class="info-block">
            <div class="info-row">
                <span class="info-label">Основание:</span>
                <span class="info-value">Заказ {{ $order['order_number'] }}</span>
            </div>
        </div>
    </div>
    ...
    <table class="items-table">
        ...
        <tfoot>
            <tr>
                ...
                <td></td>
                <td></td>
                <td class="sum" style="font-weight: bold;">{{ number_format($order['total_amount'], 2, '.', ' ') }}</td>
            </tr>
        </tfoot>
    </table>

    <div class="summary">
        <strong>Всего отпущено {{ (int)$totalQty }} наименований на сумму {{ number_format($order['total_amount'], 2, '.', ' ') }} {{ $order['currency'] }}</strong>
    </div>
