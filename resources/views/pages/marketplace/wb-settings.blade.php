            ...
            <div class="px-6 py-4">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <a href="/marketplace/{{ $accountId }}/wb-orders"
...
<div class="pwa-only min-h-screen" x-data="wbSettingsPwa()" style="background: linear-gradient(180deg, #CB11AB 0%, #9B0D83 100%);">
    ...
    <header class="text-white" style="padding-top: env(safe-area-inset-top, 20px);">
        <div class="px-4 py-3 flex items-center justify-between">
            <a href="/marketplace/{{ $accountId }}" class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
            </a>
    ...
    return {
        ...
        async loadSettings() {
            ...
            try {
                ...
                const res = await this.authFetch(`/api/marketplace/wb/accounts/{{ $accountId }}/settings?company_id=${companyId}`);
                if (res.ok) {
        ...
        async saveSettings() {
            ...
            try {
                ...
                const res = await this.authFetch('/api/marketplace/wb/accounts/{{ $accountId }}/settings', {
                    method: 'PUT',
        ...
        async testConnection() {
            ...
            try {
                const companyId = this.$store.auth.currentCompany.id;
                const res = await this.authFetch(`/api/marketplace/wb/accounts/{{ $accountId }}/test?company_id=${companyId}`, {
                    method: 'POST'
                });
...
function wbSettingsPwa() {
    return {
        ...
        async loadSettings() {
            ...
            try {
                const companyId = this.$store.auth.currentCompany.id;
                const res = await fetch(`/api/marketplace/wb/accounts/{{ $accountId }}/settings?company_id=${companyId}`, {
        ...
        async testConnection() {
            ...
            try {
                const companyId = this.$store.auth.currentCompany.id;
                const res = await fetch(`/api/marketplace/wb/accounts/{{ $accountId }}/test?company_id=${companyId}`, {
                    method: 'POST',
                    headers: this.getAuthHeaders(),
                    credentials: 'include'
                });
