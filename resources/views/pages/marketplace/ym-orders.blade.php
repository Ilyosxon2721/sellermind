                        ...
                        <div x-show="order.status === 'PROCESSING'" class="px-5 py-3 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <button @click.stop="downloadLabel(order.order_id)"
                                        :disabled="downloadingLabelId === order.order_id"
                                        class="px-3 py-1.5 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-100 flex items-center space-x-1 disabled:opacity-50">
                                    <svg x-show="downloadingLabelId !== order.order_id" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                                    </svg>
                                    <svg x-show="downloadingLabelId === order.order_id" class="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    <span x-text="downloadingLabelId === order.order_id ? 'Загрузка...' : '1 ярлык'"></span>
                                </button>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="border-t border-gray-200 pt-4">
                        <p class="text-xs text-gray-500 uppercase tracking-wide mb-3">Печать документов</p>
                        <div class="flex flex-wrap gap-2">
                            <a :href="'/api/marketplace/yandex-market/accounts/{{ $accountId }}/orders/' + selectedOrder?.order_id + '/print/receipt'"
                               target="_blank"
                               class="inline-flex items-center px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 transition">
                                <svg class="w-4 h-4 mr-1.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"/></svg>
                                Чек
                            </a>
                            <a :href="'/api/marketplace/yandex-market/accounts/{{ $accountId }}/orders/' + selectedOrder?.order_id + '/print/waybill'"
                               target="_blank"
                               class="inline-flex items-center px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 transition">
                            ...
                            <a :href="'/api/marketplace/yandex-market/accounts/{{ $accountId }}/orders/' + selectedOrder?.order_id + '/print/invoice'"
                               target="_blank"
                               class="inline-flex items-center px-3 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 transition">
                                <svg class="w-4 h-4 mr-1.5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"/></svg>
                                Счёт-фактура
                            </a>
                        </div>
                    </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
                ...
                <div class="flex justify-end space-x-3 mt-6">
                    ...
                    <button @click="saveBox()" :disabled="savingBox" class="px-4 py-2 bg-[#FFCC00] hover:bg-[#FFD633] text-black rounded-lg font-medium disabled:opacity-50 flex items-center">
                        <svg x-show="savingBox" class="animate-spin h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        <span x-text="savingBox ? 'Сохранение...' : 'Сохранить'"></span>
                    </button>
                </div>
            </div>
        </div>
...
function ymOrdersPage() {
    return {
        ...
        async downloadLabel(orderId) {
            this.downloadingLabelId = orderId;
