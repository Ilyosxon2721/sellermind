            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4">
                <div class="flex items-center justify-between flex-wrap gap-2 sm:gap-3">
                    ...
                    <div class="flex items-center flex-wrap gap-2 sm:gap-3">
                        ...
                        <!-- Bulk Stock Management Button -->
                        <button @click="openStockModal()"
                                class="px-3 sm:px-4 py-2 bg-white border-2 border-amber-500 text-amber-600 hover:bg-amber-500 hover:text-white rounded-xl text-sm font-medium transition flex items-center space-x-1 sm:space-x-2"
                                title="Управление остатками FBS/DBS">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                            </svg>
                            <span class="hidden sm:inline">Остатки</span>
                        </button>
...
<div class="flex items-center flex-wrap gap-2">
    ...
    <div x-show="showStockModal" x-cloak
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-amber-50 to-orange-50">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-lg font-bold text-gray-900 flex items-center space-x-2">
                            ...
                            <span>Управление остатками FBS/DBS</span>
                        </h2>
                        <p class="text-sm text-gray-500 mt-1" x-text="'Загружено: ' + stockItems.length + ' товаров'"></p>
                    </div>
                ...
                <div class="flex flex-wrap items-center gap-2 mt-3">
                    <button @click="selectAllStock()" class="px-3 py-1.5 text-xs font-medium bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                        <span x-text="selectedStockIds.length === stockItems.length ? 'Снять все' : 'Выбрать все'"></span>
                    </button>
                ...
                <div class="flex items-center gap-2 mt-3">
                    <input type="text" x-model="stockSearch" placeholder="Поиск по названию, баркоду..."
                    ...
                    <select x-model="stockFilter" class="px-3 py-1.5 border border-gray-300 rounded-lg text-sm">
                        ...
                        <option value="both">FBS + DBS</option>
                            ...
                            <tr class="hover:bg-amber-50/30 transition" :class="selectedStockIds.includes(item.skuId) ? 'bg-amber-50' : ''">
                                ...
                                <td class="px-4 py-2.5 text-center">
                                    <span class="font-semibold text-sm"
                                          :class="item.amount > 0 ? 'text-green-600' : 'text-red-500'"
                                          x-text="item.amount ?? 0"></span>
                                </td>
                                <td class="px-4 py-2.5 text-center">
            ...
            <!-- Footer -->
            <div class="px-6 py-3 border-t border-gray-200 bg-gray-50 flex items-center justify-between text-sm text-gray-500">
...
function uzumOrdersPage() {
    return {
        ...
        get filteredStockItems() {
            ...
            else if (this.stockFilter === 'both') items = items.filter(i => i.fbsLinked && i.dbsLinked);
        ...
        async loadStockItems() {
            ...
            try {
                const res = await this.authFetch(`/api/marketplace/uzum/accounts/${this.accountId}/stocks`);
                if (res.ok) {
                    const data = await res.json();
                    this.stockItems = data.items || [];
                } else {
                    const err = await res.json().catch(() => ({}));
                    this.stockMessage = err.message || 'Ошибка загрузки остатков';
                    this.stockSuccess = false;
                }
            } catch (e) {
                this.stockMessage = 'Ошибка сети';
                this.stockSuccess = false;
            }
        ...
        selectAllStock() {
            ...
            if (this.selectedStockIds.length === filtered.length) {
                this.selectedStockIds = [];
            } else {
                this.selectedStockIds = filtered.map(i => i.skuId);
            }
        },

        async bulkStockAction(mode) {
            if (this.selectedStockIds.length === 0) return;
            ...
            this.stockMessage = '';
            try {
                const itemsData = this.stockItems
                    .filter(i => this.selectedStockIds.includes(i.skuId))
                ...
                const res = await this.authFetch(`/api/marketplace/uzum/accounts/${this.accountId}/stocks/bulk-disable`, {
                    method: 'POST',
        ...
        async bulkStockEnable(mode) {
            ...
            try {
                ...
                const res = await this.authFetch(`/api/marketplace/uzum/accounts/${this.accountId}/stocks/bulk-enable`, {
                    method: 'POST',
                    headers: { ...this.getAuthHeaders(), 'Content-Type': 'application/json' },
                    body: JSON.stringify({ sku_ids: this.selectedStockIds, mode, items_data: itemsData }),
                });
        ...
        getStatusCount(status) {
            ...
            // Фильтруем по текущему режиму FBS/DBS
            let modeOrders = this.orders || [];
            if (this.orderMode === 'fbs') {
                modeOrders = modeOrders.filter(o => {
                    const scheme = (o.delivery_type || o.deliveryType || o.scheme || 'FBS').toUpperCase();
                    return scheme === 'FBS';
                });
            } else if (this.orderMode === 'dbs') {
                modeOrders = modeOrders.filter(o => {
                    const scheme = (o.delivery_type || o.deliveryType || o.scheme || '').toUpperCase();
                    return scheme === 'DBS' || scheme === 'EDBS';
                });
            }
