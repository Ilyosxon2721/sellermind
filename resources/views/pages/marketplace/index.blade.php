                                    ...
                                    <template x-for="account in getMarketplaceAccounts(marketplace.code)" :key="account.id">
                                        <div @click="window.location.href = '/marketplace/' + account.id"
                                            ...
                                            <div class="space-y-2">
                                                ...
                                                <!-- Кнопки для Uzum -->
                                                <div class="flex space-x-2" x-show="normalizeMarketplace(account.marketplace) === 'uzum'">
                                                    <a :href="'/marketplace/' + account.id + '/uzum-reviews'"
                                                       @click.stop
                                                       class="flex-1 px-3 py-2 mp-btn-secondary-uzum text-sm font-medium rounded-lg text-center transition flex items-center justify-center gap-1.5">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"/>
                                                        </svg>
                                                        Отзывы
                                                    </a>
                                                    <a :href="'/marketplace/' + account.id + '/uzum-invoices'"
                                                       @click.stop
                                                       class="flex-1 px-3 py-2 mp-btn-secondary-uzum text-sm font-medium rounded-lg text-center transition flex items-center justify-center gap-1.5">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
