...
function kpiPage() {
    ...
    return {
        ...
        tabLabels: {
            dashboard: @json(__('kpi.tabs.dashboard')),
            plans: @json(__('kpi.tabs.plans')),
            spheres: @json(__('kpi.tabs.spheres')),
            scales: @json(__('kpi.tabs.scales')),
        },
        ...
        async api(url, method = 'GET', body = null) {
            const opts = { method, headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content } };
        ...
        statusLabel(s) {
            return { active: @json(__('kpi.statuses.active')), calculated: @json(__('kpi.statuses.calculated')), approved: @json(__('kpi.statuses.approved')), cancelled: @json(__('kpi.statuses.cancelled')) }[s] || s;
        },
        ...
        bonusTypeLabel(t) {
            return { fixed: @json(__('kpi.scales.type_fixed')), percent_revenue: @json(__('kpi.scales.type_percent_revenue')), percent_margin: @json(__('kpi.scales.type_percent_margin')) }[t] || t;
        },

        // Загрузка данных
        async loadDashboard() {
            try {
                const res = await this.api(`kpi/dashboard?year=${this.year}&month=${this.month}`);
                this.dashboard = res.data ?? res;
            } catch (e) { this.notify(e.message, 'error'); }
        },
        async loadPlans() {
            try {
                const res = await this.api(`kpi/plans?year=${this.year}&month=${this.month}`);
                this.plans = res.data ?? [];
            } catch (e) { this.notify(e.message, 'error'); }
        },
        async loadSpheres() {
            try {
                const res = await this.api('kpi/spheres');
                this.spheres = res.data ?? [];
            } catch (e) { this.notify(e.message, 'error'); }
        },
        async loadScales() {
            try {
                const res = await this.api('kpi/bonus-scales');
        ...
        editScale(sc) {
            this.scaleForm = { ...sc, tiers: (sc.tiers ?? []).map(t => ({...t})) };
            this.showScaleModal = true;
        },
        addTier() {
