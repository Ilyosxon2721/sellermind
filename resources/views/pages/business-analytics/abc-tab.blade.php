...
<div>
    {{-- Загрузка --}}
    <template x-if="loading && !abcLoaded">
        <div class="space-y-4">
    ...
    <template x-if="abcLoaded">
        <div class="space-y-6">
                ...
                <div class="px-6 py-4 border-t border-gray-100">
                    ...
                    <div class="flex h-8 rounded-lg overflow-hidden">
                        <div class="bg-green-500 flex items-center justify-center text-white text-xs font-bold transition-all duration-500"
                             :style="'width: ' + (abcData.summary.categories.A?.percentage || 80) + '%'">
                        ...
                        <div class="bg-yellow-500 flex items-center justify-center text-white text-xs font-bold transition-all duration-500"
                             :style="'width: ' + (abcData.summary.categories.B?.percentage || 15) + '%'">
                        ...
                        <div class="bg-red-500 flex items-center justify-center text-white text-xs font-bold transition-all duration-500"
                             :style="'width: ' + (abcData.summary.categories.C?.percentage || 5) + '%'">
                            <span x-text="'C: ' + (abcData.summary.categories.C?.percentage || 0) + '%'"></span>
                        </div>
                    </div>
                    <div class="flex mt-1">
                        <div class="text-xs text-gray-500 transition-all" :style="'width: ' + (abcData.summary.categories.A?.percentage || 80) + '%'">
                            <span x-text="(abcData.summary.categories.A?.assortment_percentage || 20) + '% товаров'"></span>
                        </div>
                        <div class="text-xs text-gray-500 transition-all" :style="'width: ' + (abcData.summary.categories.B?.percentage || 15) + '%'">
                            <span x-text="(abcData.summary.categories.B?.assortment_percentage || 30) + '% товаров'"></span>
                        </div>
                        <div class="text-xs text-gray-500 transition-all" :style="'width: ' + (abcData.summary.categories.C?.percentage || 5) + '%'">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <template x-if="abcData.products.length > 50">
                    <div class="px-6 py-3 text-center text-sm text-gray-500 border-t border-gray-100">
                        Показано 50 из <span x-text="abcData.products.length"></span> товаров
                    </div>
                </template>
                <template x-if="abcData.products.length === 0">
