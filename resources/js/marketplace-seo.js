/**
 * AI SEO mixin — подмешивается в Alpine.js компоненты страниц товаров маркетплейса.
 * Использование: Object.assign(return { ... }, marketplaceSeoMixin())
 */
        ...
        async runSeoBoth() {
            ...
            try {
                const [resRu, resUz] = await Promise.all([
                    fetch(`/api/marketplace/products/${this.selectedProductForSeo.id}/seo-optimize`, {
                ...
                this.seoBothLoading = false;
            }
        },

        // --- применить название к товару ---
        async applyTitle() {
            ...
            try {
                ...
                if (this.products) {
                    ...
                    if (idx !== -1) {
                        this.products[idx].title = this.seoResult.title;
                        if (typeof this.applyFilter === 'function') this.applyFilter();
                    }
                }
