...
$__uzumShopsJson = ($uzumShops ?? collect())
    ...
    ->values();
?>
         uzumShops: <?php echo \Illuminate\Support\Js::from($__uzumShopsJson ?? [])->toHtml() ?>,
        selectedShopIds: [],
        shopAccordionOpen: false,
         ...
         fboLoading: false,
         accountId: <?php echo e($accountId); ?>,
         accountMarketplace: '<?php echo e($accountMarketplace ?? 'wb'); ?>',
         accountName: '<?php echo e(addslashes($accountName ?? '')); ?>',
         defaultCurrency: '<?php echo e(($accountMarketplace ?? 'wb') === 'uzum' ? 'UZS' : 'RUB'); ?>',
         isWb() {
        ...
        timeLeft(value) {
         ...
         setupWebSocketListeners() {
             ...
             if (accounts.length === 0) {
                 accounts.push(<?php echo e($accountId); ?>);
             }
            ...
            window.addEventListener('websocket:connected', subscribeToChannels, { once: true });
             ...
             window.addEventListener('websocket:message', (e) => {
                ...
                if (event === 'uzum.order.updated') {
                    ...
                    if (data.marketplace_account_id === <?php echo e($accountId); ?>) {
        ...
        async loadOrders(silent = false) {
            ...
            try {
                let url = '/api/marketplace/orders?company_id=' + this.$store.auth.currentCompany.id + '&marketplace_account_id=<?php echo e($accountId); ?>';
        ...
        async loadStats() {
            ...
            try {
                let url = '/api/marketplace/orders/stats?company_id=' + this.$store.auth.currentCompany.id + '&marketplace_account_id=<?php echo e($accountId); ?>';
                if (this.dateFrom) url += '&from=' + this.dateFrom;
                if (this.dateTo) url += '&to=' + this.dateTo;
                if (this.accountMarketplace === 'uzum' && this.selectedShopIds.length > 0) url += '&shop_id=' + this.selectedShopIds.join(',');
            ...
            try {
                const response = await axios.get('/api/marketplace/supplies', {
                    ...
                    params: {
                        ...
                        marketplace_account_id: <?php echo e($accountId); ?>
        ...
        async triggerSync() {
             ...
             console.log('🚀 Starting sync for account <?php echo e($accountId); ?>');
             this.syncInProgress = true;
             this.syncProgress = 0;
             ...
             try {
                 const url = '/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/orders';
         ...
         async startLiveMonitoring() {
             try {
                 const url = '/api/marketplace/accounts/<?php echo e($accountId); ?>/monitoring/start';
                 const res = await fetch(url, {
                     method: 'POST',
                     headers: this.getAuthHeaders()
                 });
         ...
         async stopLiveMonitoring() {
             try {
                 const url = '/api/marketplace/accounts/<?php echo e($accountId); ?>/monitoring/stop';
                 const res = await fetch(url, {
                     method: 'POST',
                     headers: this.getAuthHeaders()
                 });
        ...
        <header class="bg-white border-b border-gray-200 px-6 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <a href="/marketplace/<?php echo e($accountId); ?>" class="text-gray-400 hover:text-gray-600 transition">
...
<div class="pwa-only min-h-screen" x-data="{
    ...
    async loadFboOrders() {
        ...
        this.fboLoading = true;
        try {
            let url = '/api/marketplace/orders/fbo?company_id=' + this.$store.auth.currentCompany.id + '&marketplace_account_id=<?php echo e($accountId); ?>';
            if (this.dateFrom) url += '&from=' + this.dateFrom;
            if (this.dateTo) url += '&to=' + this.dateTo;
            const res = await fetch(url, { headers: this.getAuthHeaders() });
            if (res.ok) {
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.pwa-header','data' => ['title' => 'Заказы','backUrl' => '/marketplace/' . $accountId]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Заказы','backUrl' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('/marketplace/' . $accountId)]); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\server\OSPanel\home\sellermind\resources\views\pages\marketplace\orders.blade.php ENDPATH**/ ?>