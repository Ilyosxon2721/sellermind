        ...
        <header class="wb-header">
            <div class="flex items-center justify-between max-w-full">
                <div class="flex items-center space-x-4">
                    <a href="/marketplace/<?php echo e($accountId); ?>" class="wb-back-btn">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <a href="/marketplace/<?php echo e($accountId); ?>/wb-settings" class="wb-link">
...
function wbProductsPage() {
    return {
        ...
        async loadProducts(page = 1) {
            ...
            try {
                const res = await this.authFetch(`/api/marketplace/wb/accounts/<?php echo e($accountId); ?>/products?${params.toString()}`);
                if (res.status === 401) {
        ...
        async syncProducts() {
            this.syncing = true;
            try {
                const res = await this.authFetch(`/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/products`, {
                    method: 'POST'
                });
                if (res.ok) {
        ...
        async saveProduct() {
            ...
            const url = this.isEditing
                ? `/api/marketplace/wb/accounts/<?php echo e($accountId); ?>/products/${this.form.id}`
                : `/api/marketplace/wb/accounts/<?php echo e($accountId); ?>/products`;
        ...
        async deleteProduct() {
            ...
            try {
                const res = await this.authFetch(`/api/marketplace/wb/accounts/<?php echo e($accountId); ?>/products/${this.selectedForDelete.id}`, {
                    method: 'DELETE'
                });
                if (!res.ok) {
                    const err = await res.json().catch(() => ({}));
...
<div class="pwa-only min-h-screen" x-data="wbProductsPWA()" x-init="init()" style="background: linear-gradient(180deg, #CB11AB 0%, #9B0D83 100%);">
    ...
    <header class="pwa-header-gradient">
        <div class="pwa-header-content">
            <a href="/marketplace/<?php echo e($accountId); ?>" class="pwa-back-btn" onclick="if(window.haptic) window.haptic.light()">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                </svg>
            </a>
    ...
    return {
        ...
        async loadProducts(page = 1) {
            ...
            try {
                ...
                const res = await this.authFetch('/api/marketplace/wb/accounts/<?php echo e($accountId); ?>/products?' + params);
                if (res.ok) {
                    const data = await res.json();
                    this.products = data.products || [];
        ...
        async syncProducts() {
            this.syncing = true;
            try {
                const res = await this.authFetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/products', { method: 'POST' });
                if (res.ok) {
                    await this.loadProducts(1);
                }
            } catch (e) {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\server\OSPanel\home\sellermind\resources\views\pages\marketplace\wb-products.blade.php ENDPATH**/ ?>