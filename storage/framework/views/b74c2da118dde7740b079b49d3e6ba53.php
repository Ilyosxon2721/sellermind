...
<div class="browser-only" x-data="{
         ...
         getSettingsUrl() {
             ...
             const accountId = <?php echo e($accountId); ?>;
             const settingsMap = {
                 'wb': 'wb-settings',
         ...
         async loadAccount() {
             const res = await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>', {
         ...
         async loadLogs() {
             const res = await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/logs', {
                 headers: this.getAuthHeaders()
             });
             if (res.ok) {
                 const data = await res.json();
         ...
         connectSse() {
             ...
             const url = `/api/marketplace/accounts/<?php echo e($accountId); ?>/logs/stream?token=${token}&last_id=${lastId}`;
         ...
         async syncPrices() {
             ...
             await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/prices', {
                 method: 'POST',
                 headers: this.getAuthHeaders()
             });
         ...
         async syncStocks() {
             ...
             await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/stocks', {
                 method: 'POST',
                 headers: this.getAuthHeaders()
             });
         ...
         async syncOrders() {
             ...
             await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/orders', {
                 method: 'POST',
                 headers: this.getAuthHeaders()
             });
         ...
         async syncProducts() {
             ...
             await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/products', {
                 method: 'POST',
                 headers: this.getAuthHeaders()
             });
         ...
         async syncAll() {
             ...
             await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/all', {
                 method: 'POST',
                 headers: this.getAuthHeaders()
             });
        ...
        <main class="flex-1 overflow-y-auto p-6">
            ...
            <div x-show="activeTab === 'overview'">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <a :href="'/marketplace/' + <?php echo e($accountId); ?> + '/products'"
                    ...
                    <a x-show="account?.marketplace === 'wb' || account?.marketplace === 'wildberries'"
                       :href="'/marketplace/' + <?php echo e($accountId); ?> + '/wb-products'"
                       class="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition flex items-center space-x-4">
                    ...
                    <a x-show="account?.marketplace === 'wb' || account?.marketplace === 'wildberries'"
                       :href="'/marketplace/' + <?php echo e($accountId); ?> + '/wb-settings'"
                       class="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition flex items-center space-x-4">
                    ...
                    <a x-show="account?.marketplace === 'uzum'"
                       :href="'/marketplace/' + <?php echo e($accountId); ?> + '/uzum-settings'"
                       class="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition flex items-center space-x-4">
                        <div class="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center">
                            <svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
                    ...
                    <a x-show="account?.marketplace === 'ym' || account?.marketplace === 'yandex_market'"
                       :href="'/marketplace/' + <?php echo e($accountId); ?> + '/ym-settings'"
                       class="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition flex items-center space-x-4">
                    ...
                    <a x-show="account?.marketplace === 'ozon'"
                       :href="'/marketplace/' + <?php echo e($accountId); ?> + '/ozon-products'"
                       class="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition flex items-center space-x-4">
                        <div class="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">
                            <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7l9-4 9 4-9 4-9-4zm0 6l9 4 9-4m-9-4v10"/>
                    ...
                    <!-- Ozon Settings link -->
                    <a x-show="account?.marketplace === 'ozon'"
                       :href="'/marketplace/' + <?php echo e($accountId); ?> + '/ozon-settings'"
                       class="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition flex items-center space-x-4">
                        <div class="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">
                            <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>
...
<div class="pwa-only min-h-screen" x-data="{
         ...
         async loadAccount() {
             const res = await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>', { headers: this.getAuthHeaders() });
             if (res.ok) { const data = await res.json(); this.account = data.account; }
         },
         async syncAll() {
             this.syncing.all = true;
             await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/all', { method: 'POST', headers: this.getAuthHeaders() });
             this.syncing.all = false;
         }
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.pwa-header','data' => ['title' => 'Аккаунт','backUrl' => '/marketplace']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Аккаунт','backUrl' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('/marketplace')]); ?>
    ...
    <main class="native-scroll" style="padding-top: calc(44px + env(safe-area-inset-top, 0px)); padding-bottom: calc(70px + env(safe-area-inset-bottom, 0px)); padding-left: calc(12px + env(safe-area-inset-left, 0px)); padding-right: calc(12px + env(safe-area-inset-right, 0px)); min-height: 100vh;">
        <div class="px-4 py-4 space-y-4">
            ...
            <div class="native-card">
                ...
                <div class="space-y-2">
                    <a :href="'/marketplace/' + <?php echo e($accountId); ?> + '/products'" class="block p-3 bg-gray-50 rounded-xl flex items-center justify-between">
                        <span class="native-body">Товары</span>
                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    </a>
                    <a :href="'/marketplace/' + <?php echo e($accountId); ?> + '/orders'" class="block p-3 bg-gray-50 rounded-xl flex items-center justify-between">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\server\OSPanel\home\sellermind\resources\views/pages/marketplace/show.blade.php ENDPATH**/ ?>