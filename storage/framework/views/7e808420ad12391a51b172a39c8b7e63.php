    ...
    return {
        ...
        async loadOrders() {
            ...
            try {
                ...
                const res = await fetch(`/marketplace/<?php echo e($accountId); ?>/ym-orders/json?${params}`, {
        ...
        async loadStats() {
            try {
                const res = await fetch(`/marketplace/<?php echo e($accountId); ?>/ym-orders/json?per_page=1000`, {
                    headers: this.getAuthHeaders()
                });
        ...
        async syncOrders() {
            ...
            this.syncMessage = '';
            try {
                const res = await fetch(`/api/marketplace/yandex-market/accounts/<?php echo e($accountId); ?>/sync-orders`, {
                    method: 'POST',
                    headers: this.getAuthHeaders()
                });
                const data = await res.json();
        ...
        async readyToShip(orderId) {
            ...
            try {
                const res = await fetch(`/api/marketplace/yandex-market/accounts/<?php echo e($accountId); ?>/orders/${orderId}/ready-to-ship`, {
                    method: 'POST',
                    headers: this.getAuthHeaders()
                });
                const data = await res.json();
        ...
        async downloadLabel(orderId) {
            ...
            try {
                const res = await fetch(`/api/marketplace/yandex-market/accounts/<?php echo e($accountId); ?>/orders/${orderId}/labels?format=${format}`, {
                    headers: {
                        'Authorization': 'Bearer ' + this.getToken(),
        ...
        async saveBox() {
            ...
            try {
                const res = await fetch(`/api/marketplace/yandex-market/accounts/<?php echo e($accountId); ?>/orders/${orderId}/boxes`, {
                    method: 'PUT',
                    headers: this.getAuthHeaders(),
...
<div class="pwa-only min-h-screen" x-data="{
    ...
    async loadOrders() {
        this.loading = true;
        try {
            const res = await fetch('/marketplace/<?php echo e($accountId); ?>/ym-orders/json', { headers: this.getAuthHeaders() });
            if (res.ok) {
                const data = await res.json();
    ...
    async syncOrders() {
        this.syncing = true;
        try {
            await fetch('/api/marketplace/yandex-market/accounts/<?php echo e($accountId); ?>/sync-orders', { method: 'POST', headers: this.getAuthHeaders() });
            await this.loadOrders();
        } catch (e) { console.error(e); }
        this.syncing = false;
    },
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.pwa-header','data' => ['title' => 'Заказы YM','backUrl' => '/marketplace/' . $accountId]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Заказы YM','backUrl' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('/marketplace/' . $accountId)]); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\server\OSPanel\home\sellermind\resources\views\pages\marketplace\ym-orders.blade.php ENDPATH**/ ?>