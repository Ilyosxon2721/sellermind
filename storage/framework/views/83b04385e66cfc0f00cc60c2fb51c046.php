<div x-data="employeesTab()">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="badge badge-success text-xs" x-show="employee.pivot.role === 'owner'">Владелец</span>
                                </div>
                            </td>
                        </tr>
                    </template>
                ...
                <form @submit.prevent="inviteEmployee()">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" class="form-input" x-model="inviteForm.role" required
                                   placeholder="Например: Менеджер, Бухгалтер, Оператор">
                            <p class="text-xs text-gray-500 mt-1">Укажите должность сотрудника в компании</p>
                        </div>
                    </div>
    ...
    <div x-show="showTransferModal" class="modal-overlay" @click.self="closeTransferModal()">
        <div class="modal-content max-w-md">
            <div class="modal-header">
                <h3 class="modal-title">Передать владение компанией</h3>
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
                    <div class="flex">
                        ...
                        <div class="text-sm text-yellow-800">
                            ...
                            <p>Вы передаете полный контроль над компанией. Это действие нельзя отменить. После передачи вы станете обычным сотрудником.</p>
                        </div>
                    </div>
                </div>

                <template x-if="selectedEmployeeForTransfer">
                    <div class="text-center py-4">
                        <div class="w-16 h-16 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-2xl mx-auto mb-3"
                             x-text="selectedEmployeeForTransfer.name.charAt(0).toUpperCase()">
                        </div>
                        <p class="font-medium text-lg" x-text="selectedEmployeeForTransfer.name"></p>
                        <p class="text-gray-500" x-text="selectedEmployeeForTransfer.email"></p>
                    </div>
                </template>

                <div class="flex justify-end gap-2 mt-6">
                    <button type="button" class="btn btn-ghost" @click="closeTransferModal()">Отмена</button>
                    <button type="button" class="btn btn-warning" @click="transferOwnership()" :disabled="transferring">
                        <span x-show="!transferring">Передать владение</span>
                        <span x-show="transferring">Передача...</span>
                    </button>
                </div>
            </div>
        </div>
...
function employeesTab() {
    return {
        ...
        openInviteModal() {
            ...
            this.inviteForm = { name: '', email: '', password: '', role: '' };
        ...
        openTransferModal(employee) {
            this.selectedEmployeeForTransfer = employee;
            this.showTransferModal = true;
        },

        closeTransferModal() {
            this.showTransferModal = false;
            this.selectedEmployeeForTransfer = null;
        },

        async transferOwnership() {
            if (!this.selectedEmployeeForTransfer) return;

            this.transferring = true;
            try {
                await window.api.post(`/companies/${this.selectedCompanyId}/transfer-ownership`, {
                    user_id: this.selectedEmployeeForTransfer.id
                });
                this.closeTransferModal();
                await this.loadCompanies();
                await this.loadEmployees();
                if (window.toast) {
                    window.toast.success('Владение передано');
                } else {
                    alert('Владение передано');
                }
            } catch (error) {
                console.error('Error transferring ownership:', error);
                const message = error.response?.data?.message || 'Не удалось передать владение';
                if (window.toast) {
                    window.toast.error('Ошибка: ' + message);
                } else {
                    alert('Ошибка: ' + message);
...
<?php /**PATH D:\server\OSPanel\home\sellermind\resources\views/company/tabs/employees.blade.php ENDPATH**/ ?>