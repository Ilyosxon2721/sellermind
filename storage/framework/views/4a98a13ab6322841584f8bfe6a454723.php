...
<div x-data="ozonSettingsPage()" x-init="init()" class="flex h-screen bg-gray-50 browser-only">
        ...
        <main class="flex-1 overflow-y-auto p-6">
            ...
            <div x-show="!loading" x-cloak class="max-w-2xl">
                ...
                <div class="bg-white rounded-xl border border-gray-200 p-6 mt-6">
                    <h2 class="text-lg font-semibold text-gray-900 mb-4">Автоматическая синхронизация</h2>
                    <p class="text-sm text-gray-500 mb-6">
                        Настройте автоматическую синхронизацию остатков при изменениях.
                    </p>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg" :class="!syncSettings.stock_sync_enabled && 'opacity-50'">
                            <div>
                                <p class="font-medium text-gray-900">Автосинхронизация при привязке</p>
                                <p class="text-sm text-gray-500">Обновлять остатки при привязке товара к варианту</p>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg" :class="!syncSettings.stock_sync_enabled && 'opacity-50'">
                            <div>
                                <p class="font-medium text-gray-900">Автосинхронизация при изменении</p>
                                <p class="text-sm text-gray-500">Обновлять остатки при изменении на складе</p>
                            </div>
                            <label class="relative inline-flex items-center cursor-pointer">
...
function ozonSettingsPage() {
    return {
        ...
        async testConnection() {
            ...
            try {
                const res = await fetch(`/api/marketplace/accounts/<?php echo e($accountId); ?>/test?company_id=${authStore.currentCompany.id}`, {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${authStore.token}`,
                        'Accept': 'application/json',
        ...
        async saveSettings() {
            ...
            try {
                const res = await fetch('/api/marketplace/ozon/accounts/<?php echo e($accountId); ?>/settings', {
                    method: 'PUT',
                    headers: {
                        'Authorization': `Bearer ${authStore.token}`,
                        'Accept': 'application/json',
        ...
        async syncProducts() {
            ...
            try {
                const res = await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/products', {
                    method: 'POST',
                    headers: {
        ...
        async syncPrices() {
            ...
            this.syncResult = null;
            try {
                const res = await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/prices', {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${this.$store.auth.token}`,
                        'Accept': 'application/json',
        ...
        async syncStocks() {
            ...
            this.syncResult = null;
            try {
                const res = await fetch('/api/marketplace/accounts/<?php echo e($accountId); ?>/sync/stocks', {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${this.$store.auth.token}`,
                        'Accept': 'application/json',
        ...
        async loadWarehouses() {
            ...
            try {
                const res = await fetch('/api/marketplace/ozon/accounts/<?php echo e($accountId); ?>/warehouses', {
                    headers: {
                        'Authorization': `Bearer ${this.$store.auth.token}`,
                        'Accept': 'application/json',
                        'Content-Type': 'application/json'
        ...
        async loadStockSettings() {
            try {
                const res = await fetch('/api/marketplace/ozon/accounts/<?php echo e($accountId); ?>/warehouses/mapping', {
                    headers: {
                        'Authorization': `Bearer ${this.$store.auth.token}`,
                        'Accept': 'application/json',
                        'Content-Type': 'application/json'
        ...
        async saveSyncSettings() {
            ...
            try {
                ...
                if (res.ok) {
                    alert('Настройки автосинхронизации сохранены');
                } else {
                    const data = await res.json();
    ...
    async saveSettings() {
        ...
        try {
            const res = await fetch('/api/marketplace/ozon/accounts/<?php echo e($accountId); ?>/settings', {
                method: 'PUT',
                headers: { 'Authorization': 'Bearer ' + authStore.token, 'Accept': 'application/json', 'Content-Type': 'application/json' },
                body: JSON.stringify({ company_id: authStore.currentCompany.id, client_id: this.credentials.client_id, api_key: this.credentials.api_key })
            });
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.pwa-header','data' => ['title' => 'Настройки Ozon','backUrl' => '/marketplace/' . $accountId]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Настройки Ozon','backUrl' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('/marketplace/' . $accountId)]); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php endif; ?>
    ...
    <main class="native-scroll" style="padding-top: calc(44px + env(safe-area-inset-top, 0px)); padding-bottom: calc(70px + env(safe-area-inset-bottom, 0px)); padding-left: calc(12px + env(safe-area-inset-left, 0px)); padding-right: calc(12px + env(safe-area-inset-right, 0px)); min-height: 100vh;" x-pull-to-refresh="loadAccount">
        ...
        <div class="flex space-x-2 mb-3">
            ...
            <button @click="activeTab = 'settings'" :class="activeTab === 'settings' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700'" class="flex-1 py-2 rounded-lg text-sm font-medium">Настройки</button>
            ...
            <div class="space-y-3">
                <div class="native-card">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Client ID</label>
                            <input type="text" x-model="credentials.client_id" placeholder="Введите Client ID" class="native-input w-full">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">API ключ</label>
                            <input type="password" x-model="credentials.api_key" placeholder="Введите API ключ" class="native-input w-full">
                        </div>
                    </div>
                ...
                <div class="native-card bg-blue-50">
                    <p class="text-sm text-blue-800">
                        <strong>Получить API:</strong> ЛК Ozon → Настройки → Seller API
                    </p>
                </div>
            </div>
        </template>
                ...
                <div class="native-card">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg" :class="!syncSettings.stock_sync_enabled && 'opacity-50'">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" x-model="syncSettings.auto_sync_stock_on_link" :disabled="!syncSettings.stock_sync_enabled" class="sr-only peer">
                                <div class="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600 peer-disabled:opacity-50"></div>
                            </label>
                        </div>
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg" :class="!syncSettings.stock_sync_enabled && 'opacity-50'">
                            <div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\server\OSPanel\home\sellermind\resources\views\pages\marketplace\ozon-settings.blade.php ENDPATH**/ ?>