    ...
    return {
        ...
        async loadOrders() {
            ...
            try {
                ...
                const res = await fetch(`/marketplace/<?php echo e($accountId); ?>/ozon-orders/json?${params}`, {
        ...
        async loadStats() {
            try {
                const res = await fetch(`/marketplace/<?php echo e($accountId); ?>/ozon-orders/json?per_page=1000`, {
                    headers: this.getAuthHeaders()
                });
        ...
        async syncOrders() {
            ...
            this.syncMessage = '';
            try {
                const res = await fetch(`/api/marketplace/ozon/accounts/<?php echo e($accountId); ?>/sync-orders`, {
                    method: 'POST',
                    headers: this.getAuthHeaders()
                });
                const data = await res.json();
        ...
        async loadCancelReasons() {
            try {
                const res = await fetch(`/api/marketplace/ozon/accounts/<?php echo e($accountId); ?>/cancel-reasons`, {
                    headers: this.getAuthHeaders()
                });
                if (res.ok) {
                    const data = await res.json();
        ...
        async confirmCancel() {
            ...
            try {
                const res = await fetch(`/api/marketplace/ozon/accounts/<?php echo e($accountId); ?>/orders/${this.cancelModal.order.id}/cancel`, {
                    method: 'POST',
                    headers: this.getAuthHeaders(),
                    body: JSON.stringify({
        ...
        async shipOrder(order) {
            ...
            try {
                const res = await fetch(`/api/marketplace/ozon/accounts/<?php echo e($accountId); ?>/orders/${order.id}/ship`, {
                    method: 'POST',
                    headers: this.getAuthHeaders(),
                });
        ...
        async printLabel(order) {
            try {
                const res = await fetch(`/api/marketplace/ozon/accounts/<?php echo e($accountId); ?>/orders/${order.id}/label`, {
                    method: 'POST',
                    headers: this.getAuthHeaders(),
                });
...
<div class="pwa-only min-h-screen" x-data="{
    ...
    async loadOrders() {
        this.loading = true;
        try {
            const res = await fetch('/marketplace/<?php echo e($accountId); ?>/ozon-orders/json', { headers: this.getAuthHeaders() });
            if (res.ok) {
                const data = await res.json();
    ...
    async syncOrders() {
        this.syncing = true;
        try {
            await fetch('/api/marketplace/ozon/accounts/<?php echo e($accountId); ?>/sync-orders', { method: 'POST', headers: this.getAuthHeaders() });
            await this.loadOrders();
        } catch (e) { console.error(e); }
        this.syncing = false;
    },
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.pwa-header','data' => ['title' => 'Заказы Ozon','backUrl' => '/marketplace/' . $accountId]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Заказы Ozon','backUrl' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('/marketplace/' . $accountId)]); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\server\OSPanel\home\sellermind\resources\views\pages\marketplace\ozon-orders.blade.php ENDPATH**/ ?>