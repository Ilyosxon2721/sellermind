...
<script>
    window.__productEditorData = {
        initialState: <?php echo json_encode($initialState, 15, 512) ?>,
        attributesList: <?php echo json_encode($attrListMapped, 15, 512) ?>,
        globalSizes: <?php echo json_encode($sizesData, 15, 512) ?>,
        globalColors: <?php echo json_encode($colorsData, 15, 512) ?>,
        hasId: <?php echo e($product->id ? 'true' : 'false'); ?>,
        publishUrl: '<?php echo e($product->id ? route("web.products.publish", $product) : ""); ?>'
    };
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <header class="bg-white/80 backdrop-blur-sm border-b border-gray-200/50 px-6 py-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold bg-gradient-to-r from-indigo-600 to-indigo-800 bg-clip-text text-transparent"><?php echo e($product->id ? 'Редактирование товара' : 'Создание товара'); ?></h1>
                    <p class="text-sm text-gray-500">Заполните карточку товара по шагам</p>
                </div>
                <div class="flex items-center space-x-3">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($product->id): ?>
                        <form method="POST" action="<?php echo e(route('web.products.destroy', $product)); ?>" onsubmit="return confirm('Архивировать товар?');">
                    ...
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    <a href="<?php echo e(route('web.products.index')); ?>" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl transition-colors">Назад к списку</a>
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 overflow-y-auto px-6 py-6">
            <form x-ref="form"
                  method="POST"
                  action="<?php echo e($product->id ? route('web.products.update', $product) : route('web.products.store')); ?>"
                  class="space-y-6"
                  @submit.prevent="submit">
                <?php echo csrf_field(); ?>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($product->id): ?> <?php echo method_field('PUT'); ?> <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                ...
                <section x-show="step === 6" class="space-y-6">
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 space-y-4">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full text-sm">
                                <thead><tr class="text-left text-xs text-gray-500 bg-gray-50"><th class="px-3 py-2 rounded-l-lg">Вариант</th><th class="px-3 py-2">Цена (<?php echo e($currency ?? 'UZS'); ?>)</th><th class="px-3 py-2">Старая цена (<?php echo e($currency ?? 'UZS'); ?>)</th><th class="px-3 py-2 rounded-r-lg">Закупка (<?php echo e($currency ?? 'UZS'); ?>)</th></tr></thead>
                                <tbody class="divide-y divide-gray-100">
                                    <template x-for="(variant, idx) in variants" :key="idx">
                                        <tr>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.pwa-header','data' => ['title' => ''.e($product->id ? 'Редактирование' : 'Новый товар').'','backUrl' => '/products']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['title' => ''.e($product->id ? 'Редактирование' : 'Новый товар').'','backUrl' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('/products')]); ?>
...
<?php endif; ?>
    ...
    <main class="native-scroll" style="padding-top: calc(44px + env(safe-area-inset-top, 0px)); padding-bottom: calc(100px + env(safe-area-inset-bottom, 0px)); padding-left: calc(12px + env(safe-area-inset-left, 0px)); padding-right: calc(12px + env(safe-area-inset-right, 0px)); min-height: 100vh;">
        <form x-ref="form" method="POST" action="<?php echo e($product->id ? route('web.products.update', $product) : route('web.products.store')); ?>" x-on:submit.prevent="submit">
            <?php echo csrf_field(); ?>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($product->id): ?> <?php echo method_field('PUT'); ?> <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            ...
            <div x-show="currentStep === 1" class="px-4 space-y-4">
                <div class="native-card space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="native-caption">Цена (<?php echo e($currency ?? 'UZS'); ?>)</label>
                            <input type="number" name="product[base_price]" x-model="product.base_price" class="native-input mt-1" step="0.01" placeholder="0.00">
                        </div>
                        <div>
                            <label class="native-caption">Старая цена (<?php echo e($currency ?? 'UZS'); ?>)</label>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\server\OSPanel\home\sellermind\resources\views\products\edit.blade.php ENDPATH**/ ?>