...
- [ ] #056 **[BUG]** Кнопка "Синхронизировать" на странице аккаунта вызывает неверный endpoint
  - **Где:** resources/views/pages/marketplace/show.blade.php
  - **Решение:** Проверить какой endpoint вызывается при синхронизации

- [ ] #057 **[BUG]** Аккаунт не создаёт записи в журнал синхронизации
  - **Проблема:** POST возвращает 200, но логи не создаются в БД
  - **Решение:** Проверить queue worker, проверить создание SyncLog
...
- [ ] #059 **[UX]** Нет feedback при нажатии кнопок синхронизации
  - **Проблема:** Нет spinner, нет toast, пользователь не понимает нажалась ли кнопка
  - **Решение:** Добавить loading state и toast уведомление (УЖЕ ИСПРАВЛЕНО в show.blade.php)
